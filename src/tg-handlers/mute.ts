...
import { registerCallbackQueryHandler, registerUpdatesSubscription } from './_events'
...
const matchingQueries = [QUERY_MUTE, QUERY_MUTE_CONFIRM, QUERY_UNMUTE, QUERY_MUTE_CONFIRM]

const queryRegex = new RegExp(`^(${matchingQueries.join('|')})`)

export const muteMiddleware = new Composer<BotContext>()
...
muteMiddleware.callbackQuery(queryRegex, async (ctx, next) => {
  ...
  if (queryUrl.pathname === QUERY_MUTE_CONFIRM) {
    await ctx.worker.env.MUTE.put(key, JSON.stringify(new Date()), {
      expirationTtl: 24 * 60 * 60,
    })
  } else if (queryUrl.pathname === QUERY_UNMUTE_CONFIRM) {
    await ctx.worker.env.MUTE.delete(key)
  }

  if (queryUrl.pathname === QUERY_MUTE_CONFIRM || queryUrl.pathname === QUERY_UNMUTE_CONFIRM) {
